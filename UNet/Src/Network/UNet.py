...
class UNet(nn.Module):
    def __init__(self, n_in_channel, n_out_channel):
        ...
        # Downsampling
        self.conv2d_down_0 : Conv2D = Conv2D(self.n_in_channel, 64, 3, 1, 0, 2)
        self.max_pool_0 : nn.MaxPool2d = nn.MaxPool2d(2, 2)

        self.conv2d_down_1 : Conv2D = Conv2D(64, 128, 3, 1, 0, 2)
        self.max_pool_1 : nn.MaxPool2d = nn.MaxPool2d(2, 2)

        self.conv2d_down_2 : Conv2D = Conv2D(128, 256, 3, 1, 0, 2)
        self.max_pool_2 : nn.MaxPool2d = nn.MaxPool2d(2, 2)

        self.conv2d_down_3 : Conv2D = Conv2D(256, 512, 3, 1, 0, 2)
        self.max_pool_3 : nn.MaxPool2d = nn.MaxPool2d(2, 2)

        self.conv2d_down_4 : Conv2D = Conv2D(512, 1024, 3, 1, 0, 2)
        self.max_pool_4 : nn.MaxPool2d = nn.MaxPool2d(2, 2)

        # Upsampling
        self.up_sample_0 = nn.ConvTranspose2d(1024, 512, 2, 2)
        self.conv2d_up_0 : Conv2D = Conv2D(1024, 512, 3, 1, 0, 2)

        self.up_sample_1 = nn.ConvTranspose2d(512, 256, 2, 2)
        self.conv2d_up_1 : Conv2D = Conv2D(512, 256, 3, 1, 0, 2)

        self.up_sample_2 = nn.ConvTranspose2d(256, 128, 2, 2)
        self.conv2d_up_2 : Conv2D = Conv2D(256, 128, 3, 1, 0, 2)

        self.up_sample_3 = nn.ConvTranspose2d(128, 64, 2, 2)
        self.conv2d_up_3 : Conv2D = Conv2D(128, 64, 3, 1, 0, 2)
        self.conv2d_up_4 : Conv2D = Conv2D(64, n_out_channel, 1, 1)
